...
parser.add_argument(
    '--train_or_predict', type=str, default="train")
parser.add_argument(
...
class Wide_and_Deep:
    ...
    def create_model(self):
        if self.mode == 'wide and deep':
            ...
            deep_col = self.deep_columns
            print("column num: {}".format(len(deep_col)))
            # deep_col = self.deep_columns[:16] + self.deep_columns[33:]
...
if __name__ == '__main__':
    ...
    # NOTE! seems if not trained before predict, a bug comes, so always train for 1 ecpoch before pred
    if FLAGS.train_or_predict == "train":
